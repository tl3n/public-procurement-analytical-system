...
export const INDICATOR_METADATA: Record<string, IndicatorMeta> = {
  ...
  "risk.buyer_concentration": {
    ...
    interpretation:
      "Максимальна частка одного постачальника у договорах замовника за весь завантажений період (до дати публікації цього тендера). 0–1; значення близькі до 1 свідчать про захоплення ринку.",
    valueType: "numeric",
  },
  "risk.price_deviation": {
    ...
    interpretation:
      "Знакове відносне відхилення очікуваної вартості від медіани тендерів з тим самим CPV у завантаженому періоді (строго до дати публікації). Червоний — ціна є статистичним викидом за правилом Тьюкі (1.5×IQR). NULL — менше 30 порівняльних тендерів.",
    valueType: "numeric",
  },
};
